 ...
 Web Page Requests:
  ...
  Home Page:
  		...
  		/player_info - get request (no parameters)
  			This route will handle a single query to the football_players table which will retrieve the id & name for all of ther server.js file!)

node server.js
3000 is the magic port football players.
...
//Team Stats page get
app.get("/team_stats", (req, res) => { //listening for "/team_stats"
  ...
  db.task("get-everything", task => { //task for batch processing
    return task.batch([task.any(games), task.any(wins), task.any(losses)]); //query for the 3 categories
  })
    .then(info => { //if query is successful
      res.render("pages/team_stats", { //allows rendering of the page from the link
        my_title: "Team Stats",
        game_rows: info[0],
        win: info[1][0],
        loss: info[2][0]
    ...
    .catch(err => { //catch if query fails in any way
      res.render("pages/team_stats", { //re-render page
        my_title: "Team Stats"
      });
      console.log(err);
    });
...
app.get('/player_info/post', function(req, res) {
  ...
  db.task('get-everything', task => { //batch query
        return task.batch([
            task.any(players),
